(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global['lbp-form-input'] = {}));
  ...
  if (isForced_1$1(NUMBER$1, !NativeNumber$1(' 0o1') || !NativeNumber$1('0b1') || NativeNumber$1('+0x1'))) {
    ...
    'EPSILON,isFinite,isInteger,isNaN,isSafeInteger,MAX_SAFE_INTEGER,' + 'MIN_SAFE_INTEGER,parseFloat,parseInt,isInteger').split(','), j$1 = 0, key$1; keys$1$1.length > j$1; j$1++) {
      if (has$2(NativeNumber$1, key$1 = keys$1$1[j$1]) && !has$2(NumberWrapper$1, key$1)) {
        defineProperty$8(NumberWrapper$1, key$1, getOwnPropertyDescriptor$1$1(NativeNumber$1, key$1));
      }
    }
  ...
  */
  ...
  var index$2 = {
    ...
    props: {
      ...
      disabled: index$1.boolean({
        label: 'disabled'
      }),
      fontSize: index$1.number({
        label: '字号(px)'
      }),
      placeholder: index$1.string({
        label: '提示信息'
      }),
      borderWidth: index$1.number({
        label: '边框宽度(px)'
      }),
      borderRadius: index$1.number({
        label: '圆角(px)'
      }),
      vertical: index$1.boolean(),
      lineHeight: index$1.number({
        label: '行高(px)'
      })
    }
  }; // .lb-plugin__input {
